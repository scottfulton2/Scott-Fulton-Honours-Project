...
function dataInput () {
    if (isset($_POST['submit'])) {
        ...
        if (emptyRequiredFields($discipline, $projtype) !== false) {
            header("Location: staffRequestForm.php?error=emptyinput");
            exit();
        }

        // leaving out attaching files for now - $AttachedFiles
        submit($conn, $firstname, $surname, $email, $discipline, $projtype, $accountno, $accounthold, $NatureOfReq);
...
function submit($conn, $firstname, $surname, $email, $discipline, $projtype, $accountno, $accounthold, $NatureOfReq) {
    ...
    $sql = "INSERT INTO REQUESTS (RequestorType, Email, FirstName, Surname,
						Discipline, ProjType, AccountToBeCharged, AccountHolder, 
                        NatureOfReq, RequestStatus, ApproverOneID, ApproverTwoID,
                        ApprovOneRevStatus, ApprovTwoRevStatus, DateSubmitted)
    VALUES ('Staff', '$email', '$firstname', '$surname', '$discipline', '$projtype', '$accountno',
            '$accounthold', '$NatureOfReq', 'In Review', '1', '2', 'Reviewing', 'Reviewing',
            '$dateSubmitted');";
